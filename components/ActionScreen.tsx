...
function formatDiagnosis(diagnosisData: DiagnosisData): string {
    ...
    const processCategory = (category: ProblemCategory) => {
        const categoryData = diagnosisData.checkboxes[category];
        const checkValues = Object.values(categoryData);
        // 체크박스가 하나라도 체크되었거나, '기타' 항목에 글이 입력된 경우에만 처리
        if (!checkValues.some(v => v)) return;

        result += `\n[${PROBLEM_CATEGORIES[category].title}]\n`;
        PROBLEM_CATEGORIES[category].questions.forEach(q => {
            const questionId = q.id as keyof typeof categoryData;
            result += `- ${q.label}: ${categoryData[questionId] ? "예" : "아니오"}\n`;
        });
        // '기타' 항목에 내용이 있으면, 그 내용을 추가
        if (categoryData.other) {
            result += `- 기타 관련 고민: ${categoryData.other}\n`;
        }
    };
        ...
        <div className="animate-fade-in space-y-12 md:space-y-16">
            ...
            <section className="bg-white p-6 md:p-8 rounded-2xl border border-gray-200">
                ...
                >
                    ...
                    <div>
                        <label htmlFor="guardianContact" className="block text-sm font-medium text-gray-700 mb-1">연락처 (전화번호 또는 이메일)</label>
                        <input type="text" name="guardianContact" id="guardianContact" placeholder="연락받으실 전화번호나 이메일을 입력해주세요." required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500" value={formData.guardianContact} onChange={handleChange} />
                    </div>
